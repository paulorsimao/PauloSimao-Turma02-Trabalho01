class Biblioteca {
    ...
    atualizarInformacaoLivro(id, novosDados) {
        ...
        if (livro) {
            if (novosDados.titulo && typeof novosDados.titulo === 'string') {
                livro.titulo = novosDados.titulo;
            }
            if (novosDados.ano && Number.isInteger(novosDados.ano)) {
    ...
    atualizarInformacaoMembro(id, novosDados) {
        const membro = this.buscarMembroPorId(id);
        if (membro) {
            if (novosDados.nome && typeof novosDados.nome === 'string') {
                membro.nome = novosDados.nome;
            }
        }
    }
}
